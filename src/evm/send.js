...
//
// The bot fires several txs per cycle from ONE wallet (claim → buy → airdrop).
// Nonces are assigned by ethers reading getTransactionCount from the RPC at
// send time. A load-balanced public RPC is spread across nodes with slightly
// divergent views, so right after a tx is mined the next send can read a STALE
// nonce and be built with an already-used value — the node then rejects it with
// "nonce too low" / NONCE_EXPIRED.
//
// A rejected "nonce too low" tx never entered a block, so re-sending is safe:
// each resend re-reads the nonce fresh, and a short backoff lets the lagging
// node catch up. We ONLY retry the stale-nonce family (safe to resend) — never
// "already known" / replacement errors, where a tx with that nonce is already
// in the mempool and a resend could conflict.
